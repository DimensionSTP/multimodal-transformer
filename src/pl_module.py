...
class MultiModalPlModule(pl.LightningModule):
    def __init__(self, 
        ...
        metrics = MetricCollection([F1Score(num_classes=num_classes, average=average), 
    ...
    def configure_optimizers(self):
        adam_w_optimizer = optim.AdamW(self.parameters(), 
                                       lr=self.lr)
        cosine_scheduler = optim.lr_scheduler.CosineAnnealingLR(adam_w_optimizer, 
                                                                T_max=self.t_max, 
                                                                eta_min=self.eta_min)
        return {"optimizer": adam_w_optimizer, 
                "lr_scheduler": {"scheduler": cosine_scheduler, 
